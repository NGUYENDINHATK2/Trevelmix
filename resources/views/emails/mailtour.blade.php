<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Travelvn</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
</head>
<body>
    <div class="container" style="align-items: center;  padding-top: 5%;" >
        <div class="row mb-4 text-center">
            <h1 class="text-info"><b><font style="color: rgb(195, 0, 255);">Travel</font><font style="color: rgb(13, 243, 224);">vn</font></b></h1>
        </div>
        <div class="row text-center">
            <h3>Chào Mừng Đến Với Travelvn.com</h3>
            <h4>Bạn Có 1 Tour Sắp Khởi Hành </h4>
            <h4>Mã Đặt : {{$details['body']}}</h4>
            <h4>Tên Tour : {{$details['description']}}</h4>
            <h4>Ngày Khởi Hành : {{$details['departure_day']}}</h4>
            <h4>Nhớ Đến Địa Điểm Trước 1h Nhé</h4>
            <h5>Thanks !</h5>
           
        </div>

        <div class="row text-center mt-4">
            <h4>Gửi Bởi Travelvn !</h4>
        </div>
    </div>
</body>
</html>